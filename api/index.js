...
app.post('/criar-preferencia', async (req, res) => {
  try {
    ...
    const prefData = {
      ...
      items: [{
        id: String(produto.id), title: produto.nome,
        description: `Camiseta ${produto.nome} — Tamanho ${tamanho}`,
        category_id: 'fashion', quantity: 1,
        currency_id: 'BRL', unit_price: Number(produto.preco)
      }],
      payer: {
    ...
    res.json({ pedido_id: pedidoId, init_point: resp.init_point, sandbox_init_point: resp.sandbox_init_point });
  } catch (err) {
...
async function garantirAba(sheets) {
  try {
    ...
    if (!abas.includes('Pedidos')) {
      await sheets.spreadsheets.batchUpdate({ spreadsheetId: process.env.GOOGLE_SHEET_ID,
    ...
    if (!head.data.values?.[0]?.length) {
      await sheets.spreadsheets.values.update({ spreadsheetId: process.env.GOOGLE_SHEET_ID,
        range: 'Pedidos!A1:L1', valueInputOption: 'USER_ENTERED',
